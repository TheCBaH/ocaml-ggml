...
let json jsont t = Result.get_ok @@ Jsont_bytesrw.encode_string ~format:Jsont.Indent jsont t

let%expect_test "tensors" =
  with_model (fun magika ->
      let open Ggml_model_explorer in
      let nodes = TensorId.of_graph magika in
      let attrs = List.rev @@ TensorId.fold (fun t _ l -> tensor nodes t :: l) nodes [] in
      let s = json (Jsont.list Model_explorer.KeyValueList.jsont) attrs in
      print_endline s;
      [%expect
        {|
        [
